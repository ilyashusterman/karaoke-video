...
class WebmFile extends WebmContainer {
  ...
  fixDuration(duration) {
    const segmentSection = this.getSectionById(0x18538067);
    if (!segmentSection) {
      return false;
    }

    const infoSection = segmentSection.getSectionById(0x1549a966);
    if (!infoSection) {
      return false;
    }

    const timeScaleSection = infoSection.getSectionById(0x2ad7b1);
    if (!timeScaleSection) {
      return false;
    }

    let durationSection = infoSection.getSectionById(0x4489);
    if (durationSection) {
